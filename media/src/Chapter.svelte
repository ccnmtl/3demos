...
<article {hidden}>
  ...
  <p>
    We can estimate the length by selecting a finite number <M>N =</M>
    {nVects}
    <input
      ...
      step="1"
      on:change={() => {
        addVectors(nVectsElement.value);
        nVects = nVectsElement.value;
      }}
    />
    of positions along the curve and measuring the distance between them. To wit,
    we select a partition of <M>[a, b]</M>:
  </p>
  <M display
