<script>
  ...
  import { ArrowBufferGeometry, rk4, norm1 } from "../utils.js";
  ...
  const fieldMaterial = new THREE.MeshLambertMaterial({ color: params.color });
  ...
  const setTrailColors = function (
    ...
    const colorObject = new THREE.Color(params.color);

    while (index < colorArray.length) {
      for (let j = 0; j < nCubed; j++) {
        colorArray[(start + index++) % colorArray.length] =
          1 - (i / total) * (1 - colorObject.r); // red
        colorArray[(start + index++) % colorArray.length] =
          1 - (i / total) * (1 - colorObject.g); // green
        colorArray[(start + index++) % colorArray.length] =
          1 - (i / total) * (1 - colorObject.b); // blue

        colorArray[(start + index++) % colorArray.length] =
          1 - ((i + 1) / total) * (1 - colorObject.r); // red
        colorArray[(start + index++) % colorArray.length] =
          1 - ((i + 1) / total) * (1 - colorObject.g); // green
        colorArray[(start + index++) % colorArray.length] =
          1 - ((i + 1) / total) * (1 - colorObject.b); // blue
      }
      i += 1;
    }
  };
  ...
  function updateColor() {
    fieldMaterial.color.set(params.color);
    render();
  }
